...
python ../audio_download_and_split/yt_download.py --config ../json_config/tpm_config.json
if [ $? -ne 0 ]; then
...
python ../inference_runner/run_inference_text.py --config ../json_config/tpm_config.json
if [ $? -ne 0 ]; then
...
python ../make_db_csv/make_csv.py --config ../json_config/tpm_config.json
if [ $? -ne 0 ]; then
...
python ../make_db_csv/download_doc.py --config ../json_config/tpm_config_etext.json
if [ $? -ne 0 ]; then
...
python ../make_db_csv/transfer_text.py --config ../json_config/tpm_config.json
if [ $? -ne 0 ]; then
